...
class Cross_mix(nn.Module):
    ...
    def forward(self, upre,upost):# x:12,7,7,1024
        x = self.avg_pool(torch.cat([upre,upost],dim=3).permute((0,3,1,2))) #[12, 2048, 1, 1]
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False,old=1, **kwargs):
        super().__init__()
        #  双时相的尝试
        # in_chans=6
    ...
    def forward(self, x):
        if self.old==1:
            ...
            x = self.forward_features_old1(x)
            x = self.head(x)
            '''
            temp=0
            # for i in [(0,0)]:
            #     x1 = x[:,:3]
            #     x2 = x[:,3:]
            #     x2 = torch.roll(x2, shifts=i, dims=(1, 2))
            #     x3 = torch.cat([x1,x2],1)
            #     x3 = self.forward_features_old1(x3)
            #     if i==(0, 0):
            #         temp=x3
            #     else:
            #         temp=torch.cat((temp,x3),1)
            # temp = self.se(temp)
            # B L C
              # B C 1
            # x = torch.flatten(x, 1) 
