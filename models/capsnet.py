...
class CapsLayer(nn.Module):
    ...
    def forward(self, caps_output):
        caps_output = caps_output.unsqueeze(2)#2, 359552, 1, 8
        u_predict = caps_output.matmul(self.weights)#2, 359552, 1, 64
...
class PrimaryCapsLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        out = out.permute(0, 1, 3, 4, 2).contiguous()#[2, 32, 106, 106, 8]
...
class CapsNet(nn.Module):
    def __init__(self, routing_iterations, n_classes=10):
        ...
        self.conv1 = nn.Conv2d(6, 256, kernel_size=7, stride=1)
        self.primaryCaps = PrimaryCapsLayer(256, 32, 8, kernel_size=7, stride=2)  # outputs 6*6
        self.num_primaryCaps = 32 * 106 * 106
        routing_module = AgreementRouting(self.num_primaryCaps, n_classes, routing_iterations)
        self.digitCaps = CapsLayer(self.num_primaryCaps, 8, n_classes, 16, routing_module)

    def forward(self, input):
        x = self.conv1(input)#2 256 218 218
        x = F.relu(x)
        x = self.primaryCaps(x)#2, 346112, 8
        x = self.digitCaps(x)#2, 4, 16
...
if __name__ == '__main__':
    ...
    def test():
        ...
        for data, target in test_loader:
            ...
            if args.with_reconstruction:
                output, probs = model(data, target)
                reconstruction_loss = F.mse_loss(output, data.view(-1, 784), size_average=False).data
